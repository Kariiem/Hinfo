...
(defun hinfo--terminal-start (name)
  (caddr (assoc name hinfo--terminals)))
    ...
    (let ((search-end (cdr (hinfo--next-section 1))))
      (while (re-search-forward hinfo--terminal-regex search-end t)
        (push `(,(match-string-no-properties 2)
                ...
                ,(match-beginning 1))
              hinfo--terminals))))
  (push `("error" "error" ,(hinfo--section-start "Terminals")) hinfo--terminals)
  (push `("catch" "catch" ,(hinfo--section-start "Terminals")) hinfo--terminals)
  hinfo--terminals)
...
(defun hinfo-font-lock-keywords ()
  (list
   `(,(regexp-opt (mapcar #'cadr hinfo--terminals) 'symbols) . font-lock-string-face)
